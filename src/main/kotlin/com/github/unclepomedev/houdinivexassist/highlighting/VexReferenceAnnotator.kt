...
class VexReferenceAnnotator : Annotator {
    ...
    private fun checkVariableUsage(element: VexPrimaryExpr, holder: AnnotationHolder) {
        ...
        if (resolvedElement == null) {
            ...
            holder
                .newAnnotation(HighlightSeverity.ERROR, "Unresolved variable: '$varName'")
                .range(identifier.textRange)
                .create()
            return
        }
        if (resolvedElement is VexDeclarationItem) {
            // Check whether the variable on the right-hand side is within the tree of its own
            // declaration statement.
            if (com.intellij.psi.util.PsiTreeUtil.isAncestor(resolvedElement, element, false)) {
                holder
                    .newAnnotation(
                        HighlightSeverity.ERROR,
                        "Variable '$varName' is used in its own initialization",
                    )
                    .range(identifier.textRange)
                    .create()
            }
    ...
    private fun checkFunctionCall(element: VexCallExpr, holder: AnnotationHolder) {
        ...
        // However, skip if the call is inside the variable's own initializer (e.g., float dot =
        // dot())
        val resolvedVar = VexVariableResolver.resolveVariable(element, funcName)
        if (
            resolvedVar != null &&
                !com.intellij.psi.util.PsiTreeUtil.isAncestor(resolvedVar, element, false)
        ) {
            holder
                .newAnnotation(
                    HighlightSeverity.ERROR,
                    "Variable '$funcName' cannot be called as a function",
                )
                .range(identifier.textRange)
                .create()
            return
        ...
        if (!VexFunctionResolver.isKnownFunction(funcName, containingFile)) {
            holder
                .newAnnotation(HighlightSeverity.ERROR, "Unknown VEX function: '$funcName'")
                .range(identifier.textRange)
                .create()
        }
    }

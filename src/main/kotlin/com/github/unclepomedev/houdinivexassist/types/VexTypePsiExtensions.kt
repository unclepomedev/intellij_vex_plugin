...
/**
 * Retrieves two VexExpr objects, the left and right sides, from any expression node. Returns null
 * if there are not two matching elements.
 */
...
/**
 * From the expression node, obtain the OperatorKind corresponding to the operator token it
 * possesses.
 */
internal val PsiElement.operatorKind: VexTypePromotion.OperatorKind?
    get() {
        if (this is VexAddExpr) {
            if (node.findChildByType(VexTypes.PLUS) != null)
                return VexTypePromotion.OperatorKind.ADDITIVE
            if (node.findChildByType(VexTypes.MINUS) != null)
                return VexTypePromotion.OperatorKind.SUBTRACTIVE
            return null
        }

        val opNode =
            node.getChildren(null).firstOrNull {
